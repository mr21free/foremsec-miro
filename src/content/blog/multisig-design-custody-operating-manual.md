...
Do a small test transaction first. Send a tiny amount, confirm it arrives, recover it from the new wallet, then move the rest. Every time, even when you are sure.

Verify the receiving address on the signing device screen, not on your computer screen. Clipboard swapping malware still works, because people check the wrong screen.

Never fund a wallet you have not already recovered from scratch.

Do it rested and unhurried. Panic is the real attacker in most of these stories. During the incident above, some people lost funds not to the bug but to rushed transfers into wallets they had never verified.
...
So do the logistics now, while nothing is wrong.

Generate a second set of seeds and passphrases. Build a second wallet from them. Put that material at the locations that already hold your live keys, clearly labelled as the spare, and leave it alone.

The day you need it, replacement stops being a project measured in weeks and becomes an afternoon of signing. You absorbed the transport risk on a calm Tuesday instead of during a panic. That is a tested recovery path rather than a good intention.
...
Start with being able to tell that it happened. Without that you are not responding to a compromise, you are responding to a feeling. Seal what you store in tamper evident bags and record the serial somewhere separate. A cheap tamper-evident bag turns "I have no idea whether anyone opened this" into a five second check on your next visit.
...
Decide these while you are calm. Under stress people either freeze for three days or do something irreversible in ten minutes.
...
Then the step everyone forgets. **Update your inheritance plan in the same session as the change, not later.** A rotation you do not pass on is a rotation that silently broke your inheritance plan, and your family will not discover it at a good time.
...
Rebuild the wallet from backups on a clean machine and check the addresses match. This catches quiet decay: the backup you cannot read, the descriptor you cannot find, software that no longer runs.

Sign with a key combination you never normally use.

Hand the plan to the people who would have to use it, then say nothing and watch. Watching someone fail at step three of your beautifully written instructions is the most useful hour you will spend on custody all year. I do this every few months and it has never gone the way I expected.

Write down when you last did each one. That date is the difference between a system you trust and a system you assume.
...
- Do a recovery drill. Restore from your backup on a clean machine and check the addresses match. Write down the date.
- Add a strong random passphrase, put it on steel like the seed, store it in a different location from the seed, and record the exact format you used.
- Score your own risks with the two questions, and treat every "I do not know" as a drill you owe yourself.
- Check whether you can spend without visiting one specific building. If not, fix that before anything else.
- If you run multisig across locations, build and distribute a spare wallet now, and label it clearly in your inheritance document.
- Generate your next seed with dice, offline, and destroy the working material as carefully as you protect the final backup.
